(function (PV) {
	...
	var definition = {
		...
		iconUrl: '/Scripts/app/editor/symbols/ext/Icons/sym-sendvalue.svg',
	...
	var baseUrl = "https://jlefebvre7390.osisoft.int/piwebapi/";
	...
	symbolVis.prototype.init = function (scope, elem, $http, $q) {
		...
		function getStreams(datasources) {
			...
			var datastreams = _.map(datasources, function (item) {
				...
				var path = isAttribute ? path.split('?')[0] + "|" + path.split('?')[1].split('|')[1] : path;
		...
		function getEnumConfig(streams) {
			...
			_.chain(streams)
				...
				.each(function (enumstream) {
					_.extend(enumBatchRequest,
